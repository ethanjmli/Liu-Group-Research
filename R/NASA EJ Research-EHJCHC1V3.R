...
cropLongLat_zip_census <- st_transform(crop_parameters_zip,crs = st_crs("+proj=longlat +datum=NAD83 +no_defs"))
zip_Blackpop <- st_intersection(five_counties_Blackpop,cropLongLat_zip_census)
...
######Zip Code Education#####
five_counties_HSgraduate <- get_acs(
  geography = "tract", 
  variables = "B16010_015",
  state = "GA", 
  county = c("Gwinnett","Fulton","Dekalb","Cobb","Clayton"),
  year = 2020,
  geometry = TRUE
...
five_counties_HSpercent <- five_counties_HSgraduate
five_counties_HSpercent$estimate <- (five_counties_HSgraduate$estimate/
                                          five_counties_Totalpop$estimate)*100

five_c_HSgraduate<-
  ggplot()+
  geom_sf(data=five_counties_HSgraduate, aes(fill=estimate))+
  scale_fill_distiller(palette = "RdYlBu", 
                       direction = 1)+
  labs(fill = "# of People",title = "Five Counties 2020 Total High School Graduate") +
  xlab("Longitude")+
  ylab("Latitude")+
...
five_c_HSpercent<-
  ggplot()+
  geom_sf(data=five_counties_HSpercent, aes(fill=estimate))+
  scale_fill_distiller(palette = "RdYlBu", 
                       direction = 1)+
  labs(fill = "%",title = "Five Counties 2020 Percent High School Graduate") +
  xlab("Longitude")+
  ylab("Latitude")+
...
five_c_HSpercent + five_c_HSgraduate
...
setwd("C:/Users/EJLI4/Downloads")
list.files()
AOD <- rast("MCD19A2.A2020282.h10v05.006.2020290213014.hdf")
AOD2 <- rast("MCD19A2.A2020282.h11v05.006.2020290212850.hdf")
c <- c(AOD,AOD2)
plot(c)
crs(AOD)
AOD <- project(AOD,"+proj=longlat +datum=WGS84 +no_defs") #Takes time to run
AOD2 <- project(AOD2,"+proj=longlat +datum=WGS84 +no_defs") #Takes time to run
plot(AOD)
plot(AOD2)
